...
def scan_source(source_root, dest_root, mode, exclude_dirs, exclude_exts):
    ...
    for root, dirs, filenames in os.walk(source_root):
        ...
        for filename in sorted(filenames):
            ...
            if dest_folder == "NeedsReview" and class_source == "needs_user_classification":
                ext_key = ext or "(no extension)"
...
def execute_plan(plan, skip_transcribe=False, skip_format=False):
    ...
    if copied == 0:
        print("No new files copied. Nothing to process.")
        return
    ...
    if not skip_format and not skip_transcribe:
        print(f"\n{'=' * 60}")
...
- [ ] **Step 2: Test scan on a small directory**

Create a test directory and verify scanning:
```bash
mkdir -p /tmp/test-archive/Letters /tmp/test-archive/Photos
echo "test" > /tmp/test-archive/Letters/letter-2024-01-15.pdf
echo "test" > /tmp/test-archive/Photos/img001.jpg
...
### Task 2: Update Documentation and Integration
